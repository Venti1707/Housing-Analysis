...
df = pd.read_csv(
    "../Data/resale_price_index_from_1990-Q1_to_present.csv"
)
...
while True:
    ...
    if (input_summary_statistic.lower() == "mean"):
        df_grouped = df_grouped.groupby(["year"])["index"].mean().reset_index(name = "resale_price_index_summarized")
        break

    # Median
    elif (input_summary_statistic.lower() == "median"):
        df_grouped = df_grouped.groupby(["year"])["index"].median().reset_index(name = "resale_price_index_summarized")
        break
...
sns.lineplot(
    data = df_filter_pt_2,
    x = "year",
    y = "resale_price_index_summarized",
    ax = ax
)
...
ax.set_title(
    f"{input_summary_statistic.title()} resale price index from {input_minimum_year} to {input_maximum_year}",
    fontweight = "bold"
)
...
ax.set_ylabel(
    f"{input_summary_statistic.title()} resale price index",
    fontweight = "bold"
)
...
fig.savefig("../Charts/resale_price_indexes.png")
