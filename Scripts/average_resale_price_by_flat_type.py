...
# Group the data based off the year and flat type and get the mean resale price for each year and flat type
df_grouped = df_year.copy()
df_grouped = df_grouped.groupby(["year", "flat_type"])["resale_price"].mean().reset_index(name = "average_resale_price")
...
# Flat type inputs
while True:
    flat_type_input = input("Enter flat types you would like to see separated by commas, or type 'all': ")
...
ax.set_title(
    f"Average resale price for {(', '.join(selected_flat_types)).title()} flats from {input_minimum_year} to {input_maximum_year}",
    fontweight = "bold"
)
...
plt.savefig("../Charts/average_resale_price_by_flat_type.png")
